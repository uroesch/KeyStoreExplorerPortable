...
:author: Urs Roesch
:app-name-spaced: KeyStore Explorer
:app-name: KeyStoreExplorerPortable
:git-user: uroesch
...
{upstream-url}[{app-name-spaced}] is an open source GUI replacement for the Java
command-line utilities keytool and jarsigner. KeyStore Explorer presents their 
functionality, and more, via an intuitive graphical user interface.

KeyStore Explorer is written in Java and will run on any machine that has an 
Oracle JRE installed. Its capabilities are therefore available on Windows, macOS
and Linux.
